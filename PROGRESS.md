# Progreso — Bitácora: La Estancia

Actualizar marcando `[x]` cuando cada paso quede aprobado. Fecha en formato `2026-MM-DD`.

## Fase 0 — Character Reference Sheets

- [x] `rocio_ref.png` generada, guardada en `assets/refs/`. Fecha: 2026-07-28
- [x] `orion_ref.png` generada, guardada en `assets/refs/`. Fecha: 2026-07-28
- [x] `jazmin_ref.png` generada, guardada en `assets/refs/`. Fecha: 2026-07-28
- [x] Revisión conjunta aprobada. Diferenciación Rocío/Jazmín locked (gafas + largo cabello + paleta + piel). Orión: batuta en 3/4 vista es residuo, se descarta en frames de piano.

## Fase 1 — Hook 60s: Frames

- [x] `frame_01.png` — dedos temblando piano (sanity check). Generado en Krea.ai. Fecha: 2026-07-28
- [x] `frame_02.png` — Rocío reflejo ventana. Generado en Krea.ai. Fecha: 2026-07-28
- [x] `frame_03.png` — té sobre mesa. Generado en Krea.ai. Fecha: 2026-07-28
- [x] `frame_04.png` — Orión piano ojos cerrados. Generado en Krea.ai. Fecha: 2026-07-28
- [x] `frame_05.png` — Rocío silueta maletas. Generado en Nano Banana (rindió excelente en shot silueta). Bonus: añadió engranajes + reloj de pared (refuerza que es relojera). Fecha: 2026-07-28
- [x] `frame_06.png` — puerta cerradura antigua. Generado. Fecha: 2026-07-29
- [x] `frame_07a.png` — ojos Orión. Nano Banana. Fecha: 2026-07-29
- [x] `frame_07b.png` — ojos Rocío (mejor render del triptych). Nano Banana. Fecha: 2026-07-29
- [x] `frame_07c.png` — puerta entreabierta. Nano Banana. Fecha: 2026-07-29
- [x] `frame_08.png` — Rocío caminando por madera. Nano Banana. Bonus: engranajes taller de relojera visibles (consistente con frame_05). Fecha: 2026-07-29
- [x] `frame_09.png` — té slow motion (pico visual del hook). Full body funciona mejor que close-up. Fecha: 2026-07-29
- [x] `frame_10.png` — mano al hombro. Nano Banana (Krea free ya sin cuota). Bonus: taller relojería visible + cadena de reloj en chaleco Rocío. Menor: falta mancha de té en manga Orión (aceptable). Fecha: 2026-07-29
- [x] `frame_11a.png` — ojos Rocío girando lágrima. Nano Banana. Bonus: taller relojería fondo derecho + reflejo dorado iris. Fecha: 2026-07-29
- [x] `frame_11b.png` — ojos Orión al suelo. Nano Banana. Bonus: piso madera visible refuerza dirección mirada. Fecha: 2026-07-29
- [x] `frame_12.png` — freeze split (11a+11b unificado). Nano Banana. Banda blur central para overlay texto CapCut. Continuidad clavada. Fecha: 2026-07-29

## Fase 2 — Hook 60s: Animación (Kling 3.0)

- [ ] Clip 1 (frame_01 animado, 2.5s)
- [ ] Clip 2 (frame_02, 3.5s)
- [ ] Clip 3 (frame_03, 5.0s)
- [ ] Clip 4 (frame_04, 5.0s)
- [ ] Clip 5 (frame_05, 4.0s)
- [ ] Clip 6 (frame_06, 4.0s)
- [ ] Clip 7 (frame_07 triptych, 5.0s)
- [ ] Clip 8 (frame_08, 5.0s)
- [ ] Clip 9 (frame_09 slow motion, 7.0s)
- [ ] Clip 10 (frame_10, 5.0s)
- [ ] Clip 11 (frame_11 split, 7.0s)
- [ ] Clip 12 (freeze + texto, 5.0s)
- [ ] Clip 13 (logo, 2.0s)

## Fase 3 — Hook 60s: Audio

- [ ] Voz off ES grabada en ElevenLabs (`voz_off_es.mp3`)
- [ ] Voz off EN grabada en ElevenLabs (`voz_off_en.mp3`)
- [ ] Música principal generada en Udio, elegida y descargada (`musica_principal.mp3`)
- [ ] SFX descargados de freesound.org (lista en `hook_60s_plan.md`)

## Fase 4 — Hook 60s: Edición

- [ ] Ensamblaje CapCut versión ES
- [ ] Ensamblaje CapCut versión EN
- [ ] Subtítulos quemados versión ES
- [ ] Subtítulos quemados versión EN
- [ ] Color grading Shinkai aplicado
- [ ] Export 1080x1920 30fps

## Fase 5 — Publicación

- [ ] Cuenta TikTok @bitacora creada (verificar handle disponible)
- [ ] Cuenta IG @bitacora creada
- [ ] Cuenta FB creada
- [ ] Link en bio configurado (Patreon o Gumroad o Linktree)
- [ ] Publicado TikTok ES + EN
- [ ] Publicado IG Reels ES + EN
- [ ] Publicado FB Reels ES + EN

## Fase 6 — Medición (7 días post-publicación)

- [ ] Retención primeros 3s: ______%
- [ ] Watch time promedio: ______s
- [ ] Views cross-plataforma: ______
- [ ] Comentarios pidiendo continuación: ______
- [ ] CTR a link bio: ______%
- [ ] **Decisión**: verde / amarillo / rojo

## Notas de la sesión

Agregar aquí decisiones que se tomen durante la ejecución para que la próxima sesión tenga contexto sin re-leer todo:

- 2026-07-28: Setup inicial completo. Nombres locked: Rocío / Orión / Jazmín. Estética Shinkai. Voz masculina cálida. Canal: Bitácora. Paywall 3+7. Música pieza única variaciones.
- 2026-07-28: Character sheets generados en Nano Banana, aprobados. Piel neutralizada a ivory / peach.
- 2026-07-28: Nano Banana falla en frame 01 (photoreal + hands desproporcionadas). Switch a Krea.ai para frames de escena. Krea free tier + ajustar aspect ratio a 9:16.
